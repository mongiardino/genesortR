...
for(i in 1:length(gene_trees)) {
  ...
  if(type == 'AA') {
    ...
    freqs = lapply(freqs, function(x) x = x/sum(x))
    freqs = lapply(freqs, function(x) x = abs(x-mean_freqs))
    freqs = lapply(freqs, function(x) x = x/dim(gene)[1])
...
if(any(is.na(variables))) variables[which(complete.cases(variables)),]

#explore PCA patterns
summary(PCA)
loadings(PCA)[]

#attempt to find usefulness axis automatically----------------------------------------------------------------
